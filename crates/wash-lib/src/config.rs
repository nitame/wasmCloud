...
pub async fn create_nats_client_from_opts(
    ...
    let nc = if let Some(jwt_file) = jwt {
        ...
        // NOTE(nitame): Add retry_on_initial_connect because wash dev trigger an error connection refused error
        // This error happened on macos with IO error code 61, I haven't check if it happens on linux too
